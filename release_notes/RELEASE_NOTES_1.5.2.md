...
This is the first **stable** release of the Unified SDK Search refactor that previously shipped as `1.5.0b1` + `1.5.1b1`.
...
- **`searchAgents()` is the unified entry point**
  - Signature: `searchAgents(filters: Optional[SearchFilters]=None, options: Optional[SearchOptions]=None, **kwargs)`
  - `SearchParams` was replaced by `SearchFilters` + `SearchOptions`.

- **Return type (from 1.5.1b1 onward)**
  - `SDK.searchAgents()` returns `List[AgentSummary]` (pagination removed from the public API).
...
## Changes Included from 1.5.0b1 + 1.5.1b1
...
## Changes in 1.5.2 (since 1.5.1b1)
